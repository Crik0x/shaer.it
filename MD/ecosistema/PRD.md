...
La mia risposta: Fidelity universale: i punti sono associati al wallet dell'utente e sono utilizzabili presso tutti gli esercenti aderenti. Se i punti vengono riscattati presso lo stesso esercente che li ha emessi, il valore riconosciuto è del 100%. Se vengono riscattati presso un esercente diverso, il valore viene ripartito secondo percentuali configurabili dal pannello Admin (predefinite: 30% all'esercente emittente, 30% a Shareit, 40% all'esercente presso cui avviene il riscatto).
...
*Da scrivere. e da lasciare per ultimo* Hand-off via scansione QR; dati real-time operatore+dispositivo; modalità
